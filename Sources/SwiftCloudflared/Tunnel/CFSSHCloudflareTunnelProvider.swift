...
public protocol CFSSHWebSocketClient: Sendable {
    func send(data: Data) async throws
    func receive() async throws -> Data?
    func close() async
}

public protocol CFSSHWebSocketDialing: Sendable {
    func connect(request: URLRequest) async throws -> any CFSSHWebSocketClient
}

public struct CFSSHURLSessionWebSocketDialer: CFSSHWebSocketDialing {
    ...
    public func connect(request: URLRequest) async throws -> any CFSSHWebSocketClient {
        let task = session.webSocketTask(with: request)
        task.resume()
        return CFSSHURLSessionWebSocketClient(task: task)
    }
}

public final class CFSSHURLSessionWebSocketClient: @unchecked Sendable, CFSSHWebSocketClient {
...
public actor CFSSHCloudflareTunnelProvider: CFSSHTunnelProviding {
    ...
    private let requestBuilder: CFSSHAccessRequestBuilder
    private let websocketDialer: any CFSSHWebSocketDialing
    ...
    public init(
        requestBuilder: CFSSHAccessRequestBuilder = CFSSHAccessRequestBuilder(),
        websocketDialer: any CFSSHWebSocketDialing = CFSSHURLSessionWebSocketDialer(),
        originURLResolver: @escaping OriginURLResolver = { hostname in
            try CFSSHURLTools.normalizeOriginURL(from: hostname)
        },
    ...
    public func open(hostname: String, authContext: CFSSHAuthContext, method: CFSSHAuthMethod) async throws -> UInt16 {
        ...
        let websocketURL = try CFSSHURLTools.websocketURL(from: originURL)
    ...
    private func startBridge(
        clientFD: Int32,
        websocketURL: URL,
        authContext: CFSSHAuthContext,
        method: CFSSHAuthMethod,
        requestBuilder: CFSSHAccessRequestBuilder,
        websocketDialer: any CFSSHWebSocketDialing
    ) async {
    ...
    private static func runBridge(
        ...
        let websocketClient: any CFSSHWebSocketClient
        do {
    ...
    private static func pumpClientToWebSocket(clientFD: Int32, websocketClient: any CFSSHWebSocketClient) async {
    ...
    private static func pumpWebSocketToClient(clientFD: Int32, websocketClient: any CFSSHWebSocketClient) async {
        while !Task.isCancelled {
            let payload: Data
            do {
...
#if DEBUG
extension CFSSHCloudflareTunnelProvider {
    static func _testPumpClientToWebSocket(clientFD: Int32, websocketClient: any CFSSHWebSocketClient) async {
        await pumpClientToWebSocket(clientFD: clientFD, websocketClient: websocketClient)
    }

    static func _testPumpWebSocketToClient(clientFD: Int32, websocketClient: any CFSSHWebSocketClient) async {
        await pumpWebSocketToClient(clientFD: clientFD, websocketClient: websocketClient)
    }
}
#endif
