...
public protocol CFSSHClock: Sendable {
    var now: Date { get }
}

public struct CFSSHSystemClock: CFSSHClock {
    public init() {}
...
public struct CFSSHJWTValidator: Sendable {
    private let clock: any CFSSHClock

    public init(clock: any CFSSHClock = CFSSHSystemClock()) {
        self.clock = clock
    }
    ...
    public func expirationDate(from token: String) throws -> Date {
        ...
        guard parts.count >= 2 else {
            throw CFSSHFailure.auth("token is not a JWT")
        }
        ...
        guard
            ...
            throw CFSSHFailure.auth("token missing exp claim")
        }
    ...
    private func decodeBase64URL(_ value: String) throws -> Data {
        ...
        guard let data = Data(base64Encoded: base64) else {
            throw CFSSHFailure.auth("invalid JWT payload encoding")
        }
