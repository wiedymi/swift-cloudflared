...
final class CFSSHLoopbackTunnelProviderTests: XCTestCase {
    func testOpenAndCloseLifecycle() async throws {
        let tunnel = CFSSHLoopbackTunnelProvider()
    ...
    func testFaultInjectionSocketFailure() async {
        let tunnel = CFSSHLoopbackTunnelProvider(faultInjection: .socket)
        do {
            _ = try await tunnel.open(
                hostname: "ssh.example.com",
                authContext: .appToken("jwt"),
    ...
    func testFaultInjectionInetPtonFailure() async {
        let tunnel = CFSSHLoopbackTunnelProvider(faultInjection: .inetPton)
        do {
            _ = try await tunnel.open(
                hostname: "ssh.example.com",
                authContext: .appToken("jwt"),
    ...
    func testFaultInjectionBindFailure() async {
        let tunnel = CFSSHLoopbackTunnelProvider(faultInjection: .bind)
        do {
            _ = try await tunnel.open(
                hostname: "ssh.example.com",
                authContext: .appToken("jwt"),
    ...
    func testFaultInjectionListenFailure() async {
        let tunnel = CFSSHLoopbackTunnelProvider(faultInjection: .listen)
        do {
            _ = try await tunnel.open(
                hostname: "ssh.example.com",
                authContext: .appToken("jwt"),
    ...
    func testFaultInjectionGetSockNameFailure() async {
        let tunnel = CFSSHLoopbackTunnelProvider(faultInjection: .getsockname)
        do {
            _ = try await tunnel.open(
                hostname: "ssh.example.com",
                authContext: .appToken("jwt"),
