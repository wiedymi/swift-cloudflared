...
final class CFSSHAppInfoTests: XCTestCase {
    func testParseFromLoginRedirect() throws {
        ...
        let response = try XCTUnwrap(
            HTTPURLResponse(
                ...
                httpVersion: nil,
                headerFields: [
                    CFSSHAccessHeader.appDomain: "ssh.example.com"
                ]
            )
        )

        let info = try CFSSHAppInfoParser.parse(requestURL: requestURL, response: response)
        XCTAssertEqual(info, CFSSHAppInfo(authDomain: "team.cloudflareaccess.com", appAUD: "aud123", appDomain: "ssh.example.com"))
    }
    ...
    func testParseFromAUDHeader() throws {
        ...
        let response = try XCTUnwrap(
            HTTPURLResponse(
                ...
                headerFields: [
                    CFSSHAccessHeader.appDomain: "ssh.example.com",
                    CFSSHAccessHeader.appAUD: "aud456",
                ]
            )
        )

        let info = try CFSSHAppInfoParser.parse(requestURL: requestURL, response: response)
        XCTAssertEqual(info, CFSSHAppInfo(authDomain: "ssh.example.com", appAUD: "aud456", appDomain: "ssh.example.com"))
    }
    ...
    func testParseRejectsMissingDomainHeader() throws {
        ...
        XCTAssertThrowsError(try CFSSHAppInfoParser.parse(requestURL: requestURL, response: response))
    }
    ...
    func testParseRejectsMissingAuthDomain() throws {
        ...
        let response = try XCTUnwrap(
            HTTPURLResponse(
                ...
                statusCode: 200,
                httpVersion: nil,
                headerFields: [CFSSHAccessHeader.appDomain: "ssh.example.com", CFSSHAccessHeader.appAUD: "aud"]
            )
        )
    ...
    func testParseRejectsMissingAUDOutsideLogin() throws {
        ...
        let response = try XCTUnwrap(
            HTTPURLResponse(
                ...
                statusCode: 403,
                httpVersion: nil,
                headerFields: [CFSSHAccessHeader.appDomain: "ssh.example.com"]
            )
        )
    ...
    func testResolverBuildsHEADRequestAndParsesResponse() async throws {
        ...
        let client = MockHTTPClient(
            ...
            statusCode: 302,
            headers: [CFSSHAccessHeader.appDomain: "ssh.example.com"]
        )

        let resolver = CFSSHAppInfoResolver(client: client, userAgent: "test-agent")
...
actor MockHTTPClient: CFSSHHTTPClient {
