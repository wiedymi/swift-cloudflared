...
actor MockOAuthFlow: CFSSHOAuthFlow {
    var token: String
    var error: Error?
...
struct ClosureAuthProvider: CFSSHAuthProviding {
    let handler: @Sendable (String, CFSSHAuthMethod) async throws -> CFSSHAuthContext
...
enum TunnelOpenOutcome: Sendable, Equatable {
    case success(UInt16)
    case failure(CFSSHFailure)
}

actor ScriptedTunnelProvider: CFSSHTunnelProviding {
...
func collectStates(from stream: AsyncStream<CFSSHConnectionState>, count: Int) async -> [CFSSHConnectionState] {
    var iterator = stream.makeAsyncIterator()
    var states: [CFSSHConnectionState] = []
