...
final class LocalWebSocketTestServer: @unchecked Sendable {
    ...
    func start() throws -> UInt16 {
        ...
        guard fd >= 0 else {
            throw CFSSHFailure.transport("failed to create test websocket socket", retryable: true)
        }
        ...
        guard bindResult == 0 else {
            _ = close(fd)
            throw CFSSHFailure.transport("failed to bind test websocket socket", retryable: true)
        }

        guard listen(fd, SOMAXCONN) == 0 else {
            _ = close(fd)
            throw CFSSHFailure.transport("failed to listen on test websocket socket", retryable: true)
        }
        ...
        guard nameResult == 0 else {
            _ = close(fd)
            throw CFSSHFailure.transport("failed to read test websocket port", retryable: false)
        }
